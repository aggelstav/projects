...
from tensorflow.keras.layers import (
    ...
    GlobalAveragePooling1D, Input, Lambda, MaxPooling1D, Permute, RepeatVector,
...
class SnapshotEnsemble(Callback):
    ...
    def on_epoch_begin(self, epoch, logs={}):
        ...
        lr = self.cosine_annealing(epoch, self.epochs, self.cycles,
                                   self.lr_max)
        # set learning rate
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if epoch != 0 and (epoch + 1) % epochs_per_cycle == 0:
            # save model to file
            filename = "snapshot_model_%d.h5" % int(
                (epoch + 1) / epochs_per_cycle)
...
def fit_gru(train_x, train_y, test_x, test_y, n_epochs, callbacks):
    ...
    x = GRU(
        16, dropout=0.2, recurrent_dropout=0.2, return_sequences=True)(inputs)
...
sna = SnapshotEnsemble(n_epochs, 10, 0.01)
lookback = 16

train_x, train_y = preprocess_df(train_df, predict_cell, look_ahead, lookback)
test_x, test_y = preprocess_df(validation_df, predict_cell, look_ahead,
                               lookback)
